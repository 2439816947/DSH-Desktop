...
- **关于页全新升级**：
  - 品牌名统一为「Dsh 桌面端」
  - 新增**官方 Harness 最新版显示**：启动即后台加载，关于页可直接对比官方最新版与本地内置版本（纯信息展示，不自动升级）
  - 更新提示文案更新：温馨提示明确「该桌面端意在稳定，与官方 Harness 版本有半个版本时间的差距；注意：测试通道可能存在诸多问题，但是是内置 Harness 最新的」
  - 修复「检查更新」后官方版本行闪烁/消失问题（状态透传与重置保留）
- **新增更新通道（测试版入口）**：关于页可切换「稳定版 / 测试版」通道；切到测试版后检查更新可升级至测试版（内置最新 Harness）；测试版关于页提供「回退到稳定版」一键回退
